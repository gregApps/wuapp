 ...
 */
public class BaseInbox extends Activity {
    ...
    private String urlAdd="http://192.168.43.233/base_inbox/ajout_inbox.php";
    private String urlSupp="http://192.168.43.233/base_inbox/suppression_inbox.php";
    private String urlGet="http://192.168.43.233/base_inbox/affichage_inbox.php";
    private String urlGetByPseudoUser="http://192.168.43.233/base_inbox/getByPseudoUser_inbox.php";
    private String urlGetById="http://192.168.43.233/base_inbox/getById_inbox.php";
    ...
    public Mail add(Mail mail) throws ParseException, ExecutionException, InterruptedException {
        ...
        nameValuePair.add(new BasicNameValuePair("inbox_message", mail.getMessage()));
        nameValuePair.add(new BasicNameValuePair("inbox_subject", mail.getSubject()));
        nameValuePair.add(new BasicNameValuePair("inbox_date_envoi", mail.getDateEnvoi()));
        nameValuePair.add(new BasicNameValuePair("inbox_date_reception", mail.getDateReception()));
        nameValuePair.add(new BasicNameValuePair("inbox_sender", mail.getSender().getPseudo()));

        for (int i=0 ;i< mail.getRecipients().size();i++) {
            nameValuePair.add(new BasicNameValuePair("inbox_recipient[]", mail.getRecipients().get(i).getPseudo()));
        }
    ...
    public void delete(Mail mail){
        List<NameValuePair> nameValuePair = new ArrayList<NameValuePair>(1);
        nameValuePair.add(new BasicNameValuePair("inbox_id",String.valueOf(mail.getId())));
                    ...
                    else if (success==1)
                    {
                        ...
                        for(int j=0;j<dataValues.length();j++)
                        {
                            ...
                            for(int k=0;k<dataRE.length();k++){
                                ...
                                recipients.add(value.getString("inbox_recipient"));
                            }

                            Mail mail = new Mail(null,null);
                            mail.setId(Integer.valueOf(values.getString("inbox_id")));
                            mail.setSenderS(values.getString("inbox_sender"));
                            mail.setMessage(values.getString("inbox_message"));
                            mail.setSubject(values.getString("inbox_subject"));
                            mail.setDateReception(values.getString("inbox_date_reception"));
                            mail.setDateEnvoi(values.getString("inbox_date_envoi"));
    ...
    public ArrayList<Mail> getInboxByPseudoUser(String pseudo) throws ExecutionException, InterruptedException {
    ...
    public Mail getInboxById(int id) throws ExecutionException, InterruptedException {

        List<NameValuePair> nameValuePair = new ArrayList<NameValuePair>(1);
        nameValuePair.add(new BasicNameValuePair("inbox_id",String.valueOf(id)));
            ...
            if (jsonStr != null) {
                try {
                    ...
                    if (success==0)
                    {
                        ...
                        Mail mail = new Mail(null,null);
                        mail.setId(Integer.valueOf(jsonObj.getString("inbox_id")));
                        mail.setSenderS(jsonObj.getString("inbox_sender"));
                        mail.setMessage(jsonObj.getString("inbox_message"));
                        mail.setSubject(jsonObj.getString("inbox_subject"));
                        mail.setDateReception(jsonObj.getString("inbox_date_reception"));
                        mail.setDateEnvoi(jsonObj.getString("inbox_date_envoi"));
                        mail.setRecipientsS(recipients);
